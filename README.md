...
- `id`：菜单和 URL 使用的稳定标识，只能包含字母、数字、点、下划线和短横线。
- `label`：SwiftBar 中显示的名称。
- `group`：菜单分组名称，例如 `GPT-6` 或自定义分组。
...
- `legacy`：保留的兼容字段；GPT-6 预设设为 `false`，不会改变切换逻辑。
...
- GPT-6 预设动态生成，不再把预设硬编码在两份脚本里。
- 打开侧栏并应用最近一次成功使用的侧栏预设。
- “新任务默认配置”和“当前主线程”使用同一套预设；当前主线程切换成功后，会同步写入 `~/.codex/config.toml`，新线程会沿用该模型和推理强度。
- 当前侧栏仍不会修改 `config.toml`，并在操作前后检查内容不变。
...
- **新任务默认配置**：修改共享的 `config.toml`，之后新建的任务会使用该配置，并保留 `config.toml.swiftbar-backup` 备份。
- **当前主线程**：切换当前触发窗口的主线程；UI 验证成功后同步共享配置，因此之后新建的任务使用同一模型和推理强度。
...
主线程切换只有在 UI 模型和推理强度都回读成功后才写入 `config.toml`，并保留备份；侧栏切换不会写入该文件。整个过程不使用鼠标移动、全局鼠标监听、固定屏幕坐标或模拟鼠标点击。Codex 更新导致控件无法识别时，脚本会停止、通知并写诊断文件。
